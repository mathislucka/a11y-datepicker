...
function createPrimaryHeader(month, year, monthString, isLeftEdge, isRightEdge, groupId) {
    ...
    let text = document.createTextNode(monthString + ' ' + year)
...
function createRows(day, last) {
    ...
    if (currentDay <= last) {
        ...
        wrapper.appendChild(renderEmptyCells(7 - counter))
...
function createFirstRow (start) {
    ...
    let empties = renderEmptyCells(start)
...
function initializeDatepicker ({ locale, firstDayOfWeek, weekDayFormat }) {
    const weekdays = getDaysOfWeek(firstDayOfWeek, locale, weekDayFormat)
...
function getDaysOfWeek (start, locale, weekDayFormat) {
    let date = new Date()
    let first = findFirstDay(date, start, locale, weekDayFormat)
    return getWeekdays(first, locale, weekDayFormat)
}
